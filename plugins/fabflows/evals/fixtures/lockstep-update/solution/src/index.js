'use strict';
// Semver parsing, range matching and the flat resolver behind the lockstep CLI, plus `update`.
// The solution for the lockstep-update task: it proves the acceptance suite is passable.
...
function caret(p) {
  ...
  if (p.minor > 0) return [ge(lo), lt(V(0, p.minor + 1, 0))];
...
// U2 over two changed lists of [name, version] sorted by name: true if a beats b.
function beats(a, b) {
  if (a.length !== b.length) return a.length < b.length;
  for (let i = 0; i < a.length; i++) if (a[i][0] !== b[i][0]) return a[i][0] < b[i][0];
  for (let i = 0; i < a.length; i++) {
    const c = compareVersions(a[i][1], b[i][1]);
    if (c) return c > 0;
  }
  return false;
}

function update(manifest, lockfile, registry) {
  if (!isObject(lockfile) || lockfile.lockfileVersion !== 1) throw new TypeError('lockfile.lockfileVersion must be 1');
  if (!isObject(lockfile.root) || !isObject(lockfile.packages)) throw new TypeError('lockfile root and packages must be objects');
  const old = {};
  for (const [name, entry] of Object.entries(lockfile.packages)) {
    if (!isObject(entry) || typeof entry.version !== 'string') throw new TypeError(`lockfile entry ${name} needs a string version`);
    old[name] = entry.version;
  }
  // resolve validates the inputs and throws ResolutionError exactly when no answer exists (U3);
  // its answer is a valid lockfile, so it seeds the bound.
  const first = resolve(manifest, registry);
  const rootDeps = manifest.dependencies || {};
  const depsOf = (name, version) => (registry[name][version] && registry[name][version].dependencies) || {};
  const changedOf = (sel) => [...sel].filter(([n, v]) => old[n] !== v).sort(([a], [b]) => (a < b ? -1 : 1));
  let bestSel = new Map(Object.entries(first.packages).map(([n, e]) => [n, e.version]));
  let best = changedOf(bestSel);
  const selected = new Map();
  let changes = 0;

  // Branch and bound over the reachable packages, pruning any branch with more changes than
  // the best answer so far; ties are explored for U2.
  // ponytail: no lower bound beyond the running count; add one if registries outgrow the 10 s budget.
  function search(pending) {
    if (changes > best.length) return;
    if (!pending.length) {
      const changed = changedOf(selected);
      if (beats(changed, best)) {
        best = changed;
        bestSel = new Map(selected);
      }
      return;
    }
    const [name, ...rest] = pending;
    const ranges = [];
    if (Object.prototype.hasOwnProperty.call(rootDeps, name)) ranges.push(rootDeps[name]);
    for (const [p, v] of selected) {
      const d = depsOf(p, v);
      if (Object.prototype.hasOwnProperty.call(d, name)) ranges.push(d[name]);
    }
    const cands = Object.keys(registry[name] || {}).filter((v) => ranges.every((r) => satisfies(v, r)));
    for (const v of cands) {
      const cost = old[name] === v ? 0 : 1;
      selected.set(name, v);
      changes += cost;
      const deps = Object.entries(depsOf(name, v));
      if (deps.every(([q, r]) => !selected.has(q) || satisfies(selected.get(q), r))) {
        search([...new Set([...rest, ...deps.map(([q]) => q).filter((q) => !selected.has(q))])].sort());
      }
      selected.delete(name);
      changes -= cost;
    }
  }
  search(sortedKeys(rootDeps));

  const packages = {};
  for (const name of [...bestSel.keys()].sort()) {
    const version = bestSel.get(name);
    const dependencies = {};
    for (const dep of sortedKeys(depsOf(name, version))) dependencies[dep] = bestSel.get(dep);
    packages[name] = { version, dependencies };
  }
  const rootLock = {};
  for (const dep of sortedKeys(rootDeps)) rootLock[dep] = bestSel.get(dep);
  return { lockfileVersion: 1, root: { name: manifest.name, version: manifest.version, dependencies: rootLock }, packages };
}

module.exports = { parseVersion, compareVersions, parseRange, satisfies, maxSatisfying, resolve, update, ResolutionError };
