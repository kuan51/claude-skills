...
**Spec mode.** A brief may name a draft spec instead of a diff. Then the evidence is the spec plus the code paths it names, and there is no test command, so a missing one is not `BLOCKED`. Work the lenses the brief lists (failure modes, scaling cliffs, security gaps, contradictions between decisions, operations, data integrity, dependencies) and report under each, "none" included. Must-fix is a contradiction between decisions, a Behaviour line with no way to check it, or a security gap: unencrypted personal data, an auth bypass, an injection surface, a plaintext secret. Everything else is a note. Verdict words are unchanged.
