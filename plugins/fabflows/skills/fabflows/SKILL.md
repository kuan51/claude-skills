...
work over. `reviewer-blocked` means the review never ran: fix what `verdict.blocker`
names (a missing dependency is the user's to install), then run `fabflows:refuter`
yourself on `<baseRef>..HEAD` rather than restarting the loop -- the builder's commits
are already on the branch. The loop never merges, pushes, or reverts -- those stay
with you and the user.
