...
Prompts for checking that the two skills trigger and behave. Each specifies the skill
that should match and what a good answer looks like.
...
| 2 | "Audit this repo's docs" | `docs-warden` audit | Runs `audit.py`, shows the table, offers fixes, and doesn't change anything unasked. |
...
| 4 | "The README says three env vars but there are seven, fix it" | `docs-warden` maintain | Reads the code and lists the real seven, then proposes a specific edit. Does not rewrite the whole README. |
