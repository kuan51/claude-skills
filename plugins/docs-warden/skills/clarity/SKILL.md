...
## Rules this style adds

Most generic prose rules are already covered by the `Microsoft`, `write-good` and
`proselint` Vale packages, and `ai-tells` flags the patterns of machine-written
prose. The included `.vale.ini` loads all four alongside ours. Reimplementing
them would produce duplicate warnings on the same sentence and two sets of
thresholds to keep in step.

This style adds only the five rules nothing else covers:

| Rule | Level | Why this style adds it |
|------|-------|----------------|
| `SafetyConditionFirst` | warning | No general style checks the order of instructions. |
...
4. **Return a table** with the columns Rule, Original, Rewrite and Note.
...
`assets/vale/.vale.ini` is the starting configuration. The vocabulary it provides is
