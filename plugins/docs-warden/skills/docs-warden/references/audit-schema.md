...
audits and prints a Markdown table. With several paths, it prints one row per repo
...
costs more than the waiver does. The following rules make a waiver auditable:
...
- **A waiver for a check that does not exist is a `fail`**, not a silent no-op,
  because otherwise it reads as a control that is in place.
...
Checks return one of four states, and the distinction between the last two is
...
Each check includes a one-line `reason` and a `fix`. A finding a reader cannot act
on is noise.
...
| The generator exited non-zero | The generator itself is broken. The reason includes a tail of its stderr. |
| It exited 0 but didn't write anything at `path` | Silent generator failure, the worst kind, because a bare diff check would call it in sync. |
| The entry is invalid, uncontained, or a directory | The config is wrong, or `path` escapes the repository. The command never runs. |

**Fix:** run the command and commit, or fix the generator or the entry.
...
This check reports every match, even likely false positives. A false positive
costs a minute. A miss puts
...
`standards:eu-cra`, and others. That is what lets the cross-repo view name the
...
the rules belonging to it: for IEC 62304, that each document specifies a
...
than a silent skip: never a crash, because aborting the run would take
...
actually absent. See `standards.md`; each `standards/<id>.md` holds the
...
Also `fail` for a manifest it can read but not believe: a waiver referencing a check
