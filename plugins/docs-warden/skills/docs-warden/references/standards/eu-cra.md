...
- **September 11, 2026**: reporting obligations apply. Manufacturers must report
  actively exploited vulnerabilities and severe incidents, including for products
  already on the market.
- **December 11, 2027**: the essential requirements and conformity assessment
  apply in full.
...
The dates above, and the SBOM obligation in Annex I Part II(1), are
corroborated across several independent sources.

## No level axis

The CRA classifies products into default, important (Annex III class I and II), and
critical (Article 7) tiers, but those classes select the **conformity assessment
...
|-----------|----------|----------------|
| 1 | `docs/product-description.md`: intended purpose, plus versions affecting compliance and user information | |
| 2 | `docs/architecture/`: design and development | OSPS Baseline |
| 2 | `docs/vulnerability-handling.md`: how vulnerabilities are disclosed and handled, without the findings themselves | |
...
- **The ten-year retention obligation.** The technical documentation must be kept
  ...
  management, and a repository check cannot see it. This check does not enforce
  it. Document where those records live.
...
| None | `docs/regulatory/threat-model.md` (the OSPS Baseline requires `docs/threat-model.md`) |
