...
An overlay on any archetype, declared under `standards:` in
`.docs-warden.yml` as `iec-62304: <class>` and scaled by that safety class.
See `../standards.md` for the mechanism, and read it before adding another
standard here.

**This file describes document structure, not regulatory content.** Every template
in `../../assets/standards/iec-62304/` carries `<!-- CONSULT: regulatory lead -->` wherever a
...
These need re-confirming with the regulatory lead before anyone relies on
them:
...
- **IEC 62304 Edition 2** was in draft as of this writing. The class table below
  reflects the current edition, **IEC 62304:2006/AMD1:2015**, which is what
  `source_version` in the standards table names. This check does not detect
  that a newer edition has been published.
...
`audit.py`. Rows without the marker are checked by the `standards` check.

| Artifact | A | B | C |
|----------|---|---|---|
| `docs/regulatory/software-development-plan.md` | yes | yes | yes |
| `docs/regulatory/requirements/` (SRS, `REQ-<AREA>-NNN` IDs) | yes | yes | yes |
| `docs/architecture/arc42.md` | no | yes | yes |
| `docs/architecture/detailed-design.md` | no | no | yes |
| `docs/regulatory/verification/unit.md` | yes | yes | yes |
| `docs/regulatory/verification/integration.md`, `.../system.md` | no | yes | yes |
...
Git is where the work happens. The signed, approved, controlled record is stored in
the eQMS. Mixing up the two is the most common failure here: a repo full of unsigned
Markdown is not a design history, and an eQMS full of documents nobody can trace to
a commit is not evidence either.

Every regulated document's front matter carries `qms_record:` for this reason.
`pending` is a legitimate value. Absent is not.

### A release is a tag

`ddf-index.md` references git **tags**, never branches. A branch moves. An auditor
asking what was released needs something that does not.

### Requirements trace from the code
...
software of unknown provenance. Update this whenever the lock file changes. **Not
...
### Problem resolution is tracked externally
