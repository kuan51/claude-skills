...
| `docs/MAINTAINERS.md` | GV-01.01, .02 | no | yes | yes |
| `docs/dependencies.md` | DO-06.01 | no | yes | yes |
| `docs/how-to/build.md` | DO-07.01 | no | yes | yes |
| `docs/architecture/` | SA-01.01 | no | yes | yes |
| `docs/reference/` | SA-02.01 | no | yes | yes |
| `docs/security-assessment.md` | SA-03.01 | no | yes | yes |
| Contributor requirements **(not checked)**: content of `CONTRIBUTING.md` | GV-03.02 | no | yes | yes |
| A CVD policy with a response timeframe **(not checked)**: content of `docs/SECURITY.md` | VM-01.01 | no | yes | yes |
| A private reporting channel **(not checked)**: a repository setting rather than a file | VM-03.01 | no | yes | yes |
| Published vulnerabilities **(not checked)**: an external channel | VM-04.01 | no | yes | yes |
| `SUPPORT.md` | DO-04.01, DO-05.01 | no | no | yes |
| `docs/threat-model.md` | SA-03.02 | no | no | yes |
| `docs/how-to/verify-release.md` | DO-03.01, .02 | no | no | yes |
| `docs/remediation-policy.md` | VM-05.01, VM-06.01 | no | no | yes |
| A permission-review policy **(not checked)**: content of `docs/MAINTAINERS.md` | GV-04.01 | no | no | yes |
| VEX documents **(not checked)** | VM-04.02 | no | no | yes |

## Control families this overlay leaves out

The Baseline is broader than documentation. Some families are **deliberately
not represented**, because they govern repository settings and pipeline
configuration that this plugin does not inspect:

- **OSPS-AC**: access control, including branch protection, MFA, and token scopes.
- **OSPS-BR**: build and release, including pipeline hardening and signed releases.
...
`docs/regulatory/threat-model.md`; this overlay requires `docs/threat-model.md`.
