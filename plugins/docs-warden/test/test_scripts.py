...
def test_gen_plugin_table_is_idempotent_and_backslash_safe():
    """scripts/gen_plugin_table.py (repo root, not a docs-warden plugin script)
    must rewrite only the marked region, be a no-op on a second run, and
    survive a description containing backslashes -- re.sub interprets \\g<...>
    and numbered backreferences in a plain replacement string, which is why
    the generator uses a lambda replacement instead."""
    repo_root = Path(__file__).resolve().parents[3]  # tests -> docs-warden -> plugins -> repo root
    generator = repo_root / "scripts" / "gen_plugin_table.py"
    assert generator.is_file(), f"{generator} not found"

    with tempfile.TemporaryDirectory() as tmp:
        repo = Path(tmp)
        (repo / "scripts").mkdir()
        (repo / "scripts" / "gen_plugin_table.py").write_text(generator.read_text())
        (repo / ".claude-plugin").mkdir()
        (repo / ".claude-plugin" / "marketplace.json").write_text(
            '{"plugins": [{"name": "widget", "description": "Uses \\\\g<1> literally."}]}'
        )
        (repo / "README.md").write_text(
            "# Repo\n\n## Available Plugins\n\n"
            "<!-- BEGIN GENERATED:plugin-table - run scripts/gen_plugin_table.py -->\n"
            "<!-- END GENERATED:plugin-table -->\n"
        )

        run = lambda: subprocess.run(  # noqa: E731
            [sys.executable, "scripts/gen_plugin_table.py"],
        ...
        assert "`widget`" in text, f"plugin row missing:\n{text}"
        assert r"Uses \g<1> literally." in text, \
            f"backslash sequence was interpreted as a backreference:\n{text}"

        second = run()
        assert second.returncode == 0, second.stderr
        assert "already current" in second.stdout, \
            f"not idempotent, second run wrote again: {second.stdout}"
        assert (repo / "README.md").read_text() == text, "second run changed the file"


BEGIN = "<!-- BEGIN GENERATED:plugin-table - run scripts/gen_plugin_table.py -->"
END = "<!-- END GENERATED:plugin-table -->"
GEN = Path(__file__).resolve().parent.parent.parent.parent / "scripts" / "gen_plugin_table.py"


def _scratch(tmp, readme_body):
    """A repo the generator can run in: its own copy of the script (the script
    derives the repo root from its own location), a marketplace, and a README."""
    repo = Path(tmp)
    (repo / "scripts").mkdir()
    (repo / "scripts" / "gen_plugin_table.py").write_text(
        GEN.read_text(encoding="utf-8"), encoding="utf-8")
    (repo / ".claude-plugin").mkdir()
    (repo / ".claude-plugin" / "marketplace.json").write_text(
        '{"plugins": [{"name": "p", "description": "A plugin."}]}', encoding="utf-8")
    (repo / "README.md").write_text(readme_body, encoding="utf-8")
    return repo


def _run(repo):
    return subprocess.run(
        [sys.executable, str(repo / "scripts" / "gen_plugin_table.py")],
        cwd=repo, capture_output=True, text=True, check=False)


def test_generator_rejects_a_missing_end_marker():
    """A deleted marker must exit non-zero. audit.py judges staleness by
    before != after, so a generator that no-ops on a broken region and exits 0
    leaves the gate reporting a README it no longer maintains as in sync."""
    with tempfile.TemporaryDirectory() as tmp:
        repo = _scratch(tmp, f"# R\n\n{BEGIN}\n\nold table\n")
        r = _run(repo)
        assert r.returncode != 0, f"exited 0 with no END marker:\n{r.stdout}{r.stderr}"


def test_generator_rejects_a_duplicated_begin_marker():
    """Two BEGINs used to make the non-greedy region match span from the first
    BEGIN to the END, silently deleting the hand-written prose between them."""
    with tempfile.TemporaryDirectory() as tmp:
        prose = "PROSE THAT MUST SURVIVE"
        repo = _scratch(tmp, f"# R\n\n{BEGIN}\n\n{prose}\n\n{BEGIN}\n\nold\n{END}\n")
        r = _run(repo)
        assert r.returncode != 0, f"exited 0 with two BEGIN markers:\n{r.stdout}{r.stderr}"
        assert prose in (repo / "README.md").read_text(encoding="utf-8"), \
            "the generator ate hand-written prose between the duplicated markers"


def test_generator_rejects_reordered_markers():
    """END before BEGIN passed both `in` tests, matched nothing, and reported
    'already current' -- a no-op indistinguishable from success."""
    with tempfile.TemporaryDirectory() as tmp:
        repo = _scratch(tmp, f"# R\n\n{END}\n\nold\n{BEGIN}\n")
        r = _run(repo)
        assert r.returncode != 0, f"exited 0 with reordered markers:\n{r.stdout}{r.stderr}"


def test_generator_is_idempotent_on_a_well_formed_region():
    """The success path must survive the hardening above."""
    with tempfile.TemporaryDirectory() as tmp:
        repo = _scratch(tmp, f"# R\n\n{BEGIN}\n{END}\n")
        assert _run(repo).returncode == 0
        first = (repo / "README.md").read_text(encoding="utf-8")
        assert "`p`" in first, f"table not written:\n{first}"
        assert _run(repo).returncode == 0
        assert (repo / "README.md").read_text(encoding="utf-8") == first, "not idempotent"
