...
cares about: a skill that *under-triggers* (misses a query meant for it) and one that
...
so it must never be automatically selected. Queries about its job live in the corpus as negatives
(`expected: null`), which also verifies it stays out of the set of automatically triggered skills.
...
**With verbs as the entry point, verb confusion is the failure mode that matters most.** Naming a
...
framework name was. `test/corpus-shape.test.js` fails if the corpus stops including a
disambiguation query for each of them, but it only checks that the questions are still being asked;
whether the model answers them correctly is what the run below measures.
...
2. For each held-out query, start a **fresh** Claude Code session in a project where ciso is
   ...
   record which skill Claude automatically invokes (or none). For queries naming a certification, seed
...
4. **Score**:
   ...
   - *False positive*: a ciso skill triggers when `expected` was `null`, or the wrong
     one triggers (over-trigger / mis-route).
...
5. **Target**: >= 90% trigger accuracy with a low false-positive rate. If a skill under-triggers,
   ...
   `description` and depend on the negatives that are firing it. Re-run only after changing
