...
else.** These rules apply no matter which verb is running. A verb's own reference file not being loaded is
...
- **e1 and i1** both include `sourceAuthority: "public-topic-level"` content: topic-level structure
  ...
  included file. Explicitly non-authoritative; every entry citation-backed; a
...
- **r2** includes a small illustrative-only example set (not the real ~2000+-entry scope) pending its
...
- If an org obtains its own licensed `<tier>` MyCSF export, importing it **replaces that tier's
  `controls` map wholesale** (this plugin's synthetic topic-level ids never match real
...
This was a routing step when HITRUST had one entry point. It is an invariant now precisely
...
**Carve-outs, or the rule eats itself:**
...
- **`ciso:upgrade` is exempt.** It is the verb this check sends people to, so applying the check to
  ...
  and carry on.
- **A brand-new tier registration is exempt.** It has no prior version to upgrade from.
...
- **Keep an org's posture out of vendor research.** Justifications and in-progress notes never enter vendor research.
