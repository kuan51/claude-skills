...
Read this when `ciso:upgrade` dispatches here, that is, the plugin's bundled `controls/<tier>.v*.structure.json` is a newer version than `state.json`'s `tiers.<tier>.controlSetVersion`. Every other verb checks for this first and stops, sending the user here, because interview and roadmap data may need reconciling against the new structure before those flows touch it.
