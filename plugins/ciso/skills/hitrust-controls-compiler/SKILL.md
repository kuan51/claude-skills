...
shared e1/i1 structure files that are included inside it. An organization using `ciso:register` and `ciso:interview` never runs
this.

## Purpose

No licensed MyCSF export backs either included tier. e1's structure must come from public
information only. An org's own licensed export, if the maintainer happens to have one, may be used
purely as a private completeness cross-check. It never goes to a research agent, is never cited, and
never appears in the released output. i1 has never had a licensed export at all. A hallucinated or
incomplete control list would sit on an org's compliance dashboard looking just as authoritative as
a real one. This skill's whole design exists to prevent that: full-coverage research, then a
dedupe/reconcile pass, then two independent adversarial verification passes before anything is
released.
...
2. **Run the `Workflow` tool twice, once per tier: e1 FIRST, then i1**, with
   ...
   Each run does four phases, in order:
   - Research: one `hitrust-topic-researcher` agent per domain, full coverage for that tier.
   - Reconcile: one `hitrust-controls-reconciler` agent dedupes and flags coverage gaps, and
     separately lists domains that returned zero candidates.
   - Verify-Refute: one `hitrust-controls-verifier` pass that tries to reject each candidate.
   - Verify-Confirm: a second, independent `hitrust-controls-verifier` pass on whatever survived.
     Dual-pass because everything here is released publicly with no severity gate.
...
3. **Review before writing anything.** For each tier's run, read `coverageGaps` and `excluded`
   (candidates that failed verification, with a reason each) yourself before assembling the released
   files. The workflow's agents never get final say on what actually gets released; a human reviews it
   ...
   secondary source dominates the released list, note that as a caveat in `coverageNote` rather than
   presenting it as broad literature) and, for i1, count how many released entries include a verified
   `controlReference`. Report that fraction in `coverageNote`; it will likely be a
...
4. **Assemble the released files** (same structure for both tiers):
   - `plugins/ciso/skills/hitrust/controls/<tier>.v<version>.structure.json`: `{ tier: "e1"|"i1",
     ...
     **only when the released candidate included a verifier-confirmed `controlReference`.** Never
...
- **Never present a hallucinated or incomplete e1/i1 entry as authoritative.** Every released entry
...
- **Never skip the second (confirm) verification pass under time/cost pressure.** A candidate that
  ...
  release what refute already accepted").
