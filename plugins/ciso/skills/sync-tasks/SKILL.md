...
Turns HITRUST controls with an unresolved `assessment.status` (`gap` or `in_progress`) into real tickets in JIRA or Linear, so compliance findings become assignable engineering work instead of living only in the static dashboard pages. Invoked explicitly by the user (such as "sync my HITRUST gaps to JIRA"). This is a separate concern from the `ciso:interview` assessment flow, never run automatically as part of it.
...
2. Ask which certification (`certKey`, such as `"hitrust"`) and tier (`e1`/`i1`/`r2`) to sync, if there's more than one registered in `state.json`.
3. Read `state.certifications.<certKey>.sync.destination` (see `lib/diff-tasks.js`'s `getDestination`). If it's not set:
   - Ask which tracker (JIRA or Linear), and the destination details for that tracker. For JIRA that's project key, issue type, and whether Advanced Roadmaps is available. For Linear that's team and project.
...
7. Report a summary to the user, giving counts created, updated, closed, and reopened, with ticket links.
