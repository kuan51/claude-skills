...
`type2` is the only tier this module provides. Type I and Type II assess the *same* Trust Services
...
`reportType: "type1"`. Everything else about the flow stays the same.

**SOC 2 is a report rather than a certification, and this must always be communicated to the user.** A
...
- **For a Type II, "met" means designed AND operating effectively across the whole observation
  ...
  `in_progress`, and the current-state note should say when it started. This is the most
...
## Applying the Type II rule to evidence
