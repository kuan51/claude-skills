...
4. Sort the family's criteria by `relatedControlCode` (`CC6.1`, `CC6.2`, …) and work through them in sub-batches of 4-6. **Never accumulate a whole family before the first `ExitPlanMode`**. Families are unevenly sized (`CC6` has 8 and `P6` has 7, while `CC8`, `P1`, `P2`, `P7` and `P8` have exactly 1 each), so sub-batch by count, not by family, and for the single-criterion families, expect to cover several in one sitting rather than one plan-mode round trip each. For each criterion:
   ...
   - Then, freeform conversational follow-up:
     ...
     - **gap** / **not applicable** / **defer** -> notes encouraged, not required. For `not_applicable`, the justification should name why: an out-of-scope category, or a control actually inherited from a carve-out subservice organization.
...
For a Type II, the interview is really asking two questions per criterion. The first is easy; the second is hard:
