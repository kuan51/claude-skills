...
Read this when `ciso:roadmap` dispatches here. Available as soon as ANY completed criteria family has un-researched gaps. It does not wait for every family in the tier to finish. **Runs in the background:** launched fire-and-forget so it never blocks the interview, with findings merged in whenever it finishes.

The roadmap workflow is certification-agnostic core (see `ADDING-A-CERTIFICATION.md`). It lives under `skills/hitrust/lib/roadmap/` for historical reasons and doesn't need a SOC 2-specific variant.
...
Compliance-automation platforms (Vanta, Drata, Secureframe and similar) come up repeatedly in this research, because they automate the evidence-collection burden that makes Type II expensive. Tell the user what a vendor list won't say on its own:
...
- **Buying the platform mid-period does not retroactively create evidence.** For a Type II, the control had to be operating across the observation period. Tooling adopted in month five documents months five onward. The recommendation must address that timing, not just the price.
