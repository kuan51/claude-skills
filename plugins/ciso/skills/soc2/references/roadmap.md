...
Read this when `ciso:roadmap` dispatches here. Available as soon as ANY completed criteria family has un-researched gaps -- not gated on the whole tier finishing. **Runs in the background:** launched fire-and-forget so it never blocks the interview, with findings merged in whenever it finishes.
