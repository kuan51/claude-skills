...
Before this existed, a control marked `met` stored only a free-text justification. A human had to
retype anything a development process produced before it could appear on the dashboard, during an
interview.

## Evidence does not change status

**Attaching evidence never writes to `assessment`**: not `status`, and above all not `assessedAt`.
`record-evidence.js` enforces this mechanically and a test pins it. Assessment and evidence are independent axes:
...
If the user wants a control's *status* changed, that is `ciso:interview`. Say so rather than
...
The script appends rather than replacing. A control accumulates evidence across many PRs this way.
It also validates fail-closed: an unknown `kind` or field, or a blank `ref` or `summary`, is rejected and
