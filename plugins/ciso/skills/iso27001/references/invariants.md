...
else.** These apply no matter which verb is running. A verb's own reference file not being loaded is
...
This module also deliberately has no scope flow: `ciso:scope` does not apply to ISO 27001. SOC 2 needs
...
**This plugin never produces a certificate.** ISO 27001 certification is issued by an
accredited certification body after a Stage 1 documentation review and a Stage 2 audit of the system
in use, and maintained through annual surveillance audits and a three-year recertification. This gets
...
The bundled control set is `sourceAuthority: "public-topic-level"` and explicitly non-authoritative.
...
- **ISO/IEC 27001:2022 is sold, not published.** Its identifiers were **reconstructed from convergent
  public sources, never read from the standard.** Every entry holds `codeCorroboratedBy`,
...
- **Annex A (93 entries) is the stronger half.** It has four themes, sized 37, 8, 14 and 34, ending at A.5.37,
  ...
  outvoted. A second independent full list agrees on all 93 assignments, and the closure itself was
  ...
  so entries carry `codeCorroboratedBy` rather than `codeVerifiedBy`.
- **Clauses 4-10 (30 entries) are weaker still.** This module could not reach a primary source, and there is no
  ...
  certification body works from a different cut, treat that as a difference of convention rather than an error in
  either.
...
- **"Met" always needs a real justification; "in progress" needs both a current-state and an
  ...
  placeholder.
- **Both halves are mandatory, with no scoping fork.** Every clause applies to every ISMS, and
...
- **An org's posture never leaves the project.** Justifications and in-progress notes never enter vendor research;
