...
This merges the 123 bundled entries into `<docs/ciso-dir>/state.json` and creates the `interviewSessions` entry for `iso27001`/`isms`. Safe to re-run. It adds only control ids that are missing and creates the interview session if one doesn't already exist. It never touches an existing control's `assessment`/`roadmap`.
