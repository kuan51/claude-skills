...
**This flow reads `state.json` directly and doesn't need a separate SoA subsystem.** The Statement of Applicability records, for each of the 93 Annex A controls, whether it is included or excluded, why, and its implementation status. `state.json` already stores exactly that. This flow is a *reading* of data the interview recorded. It never writes, and it never asks a question the interview should have asked.
...
4. **Whether clause 6.1.3 is assessed at all.** The SoA follows from risk treatment rather than feeding into it. If `iso27001-6.1.3` is `not_assessed` or a `gap`, the document you are about to produce has no risk assessment supporting it, and saying so now is more useful than a tidy table.
