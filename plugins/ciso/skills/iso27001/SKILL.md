...
- **Annex A (93 entries) is the stronger half.** Four themes of 37/8/14/34 terminating at A.5.37, A.6.8, A.7.14 and A.8.34, summing to 93. Each theme's published count equals its own terminal number, so the set is arithmetically closed and a wrong source is detectable rather than merely outvoted. Two independent full lists agree on all 93 assignments. The structure file also names a publisher artifact — an official ISO/IEC 27002:2022 preview, whose numbering corresponds one-to-one with Annex A — because closure alone proves only that the published counts agree with each other, not that they are right. **Be exact if asked about it:** this project confirmed that file is reachable and is the document claimed, but did not read its contents (it is encrypted). A user who opens it can check the terminal numbers; we did not.
