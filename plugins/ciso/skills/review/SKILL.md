...
**This verb is read-only.** It never writes to `state.json`. A review's conclusions become a
recorded fact only once the user runs `ciso:evidence` or re-answers in `ciso:interview`. That separation
...
Read `--stat` first to see the outline, then the diff itself. For a large diff, read the files that
...
**Be honest about what this is: judgment, not lookup.** This plugin has no path-to-control manifest,
and deliberately none is planned: `state.json` is gitignored, and a committed mapping file
