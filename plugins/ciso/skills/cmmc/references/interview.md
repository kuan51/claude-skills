...
mode.** This is a firm requirement: each sub-batch only counts as "committed" once the
user approves it via `ExitPlanMode`.

The domain keys depend on the tier:

- `level1`: one domain, `FCI` (15 requirements). One or two sittings.
...
- `level3`: families carried over from NIST SP 800-172, a few requirements each.
...
against an organization that already has a Level 2 certification. Assessing the enhanced
requirements first produces a number that doesn't describe anything.
...
**`not_applicable` requires that the technology genuinely does not exist in scope**, for example no
wireless anywhere in the boundary, no mobile devices, no publicly accessible system components. Under the DoD
Assessment Methodology a requirement that does not apply to the environment is scored as met rather
than deducted. That is exactly why it is worth pressing on. "We don't do that yet" is a **gap**, and
"we don't think it matters" is a gap too. Push for a reason that specifies the absent technology or data
flow.
...
6. For every requirement processed in this sub-batch, run:
   ...
   the mechanical backstop, not just prose: it throws and doesn't make any changes if `status` is
...
- **3.12.4 (system security plan)**: ask whether the SSP describes the *actual* boundary, and when
  ...
  checked against it.
- **3.1.1 / 3.1.2 (access control)** and **3.5.3 (multifactor authentication)**: these fall under
  5-point requirements under the DoD Assessment Methodology and are **not** POA&M-eligible. A soft answer
