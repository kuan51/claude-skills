...
**Read-only.** It writes nothing to `state.json`. A problem it finds gets fixed elsewhere:
...
2. **Stale assessment.** `assessment.assessedAt` more than **12 months** old. Twelve months is the
   common recertification and surveillance cadence; say that you used it, since an org's cycle
...
The choice matters: `occurredAt` is when the artifact happened; `recordedAt` is
only when someone typed it in. Judging on `recordedAt` alone flags every control whose supporting
...
Close with the highest-leverage next action rather than a list of everything.
