...
persistent, local HTML dashboards you open directly in a browser (no server required). Includes
structural control metadata, plus verbatim requirement text in the one case where the publisher's
work has no copyright (CMMC's NIST and eCFR sources). Licensed or copyrighted requirement
...
- You want a one-off answer about a control. Just ask directly; the full interview flow
...
- You want to know whether **one repository** contains the documents a standard requires. That is
  ...
  evidence. That one scores a repo's document set. They don't share state, and neither reads the
  other's.
...
2. Run `ciso:init`: scaffolds `docs/ciso/state.json` and `docs/ciso/dashboard.html`, and adds
   ...
   **Under the hood:** this is your scoreboard, plain HTML files you open directly in any
   browser (no server), and everything about your organization remains local to the project.
3. Run `ciso:register`: pick a certification and load its control set. **Under the hood:** each
   ...
   both halves at once (the 30 management-system requirements from clauses 4 through 10 and all 93
   ...
   level3, because the enhanced requirements are additional to all 110 rather than replacing them.
...
6. Open `docs/ciso/dashboard.html` in any browser. That page is the **index**: one card per
   ...
   (`docs/ciso/cert-<name>.html`): per-tier gauges, per-domain progress, and a detailed view into
   ...
   **Under the hood:** every sub-batch of answers regenerates these files, so an interrupted
   session never loses more than a few controls in flight.

All certifications share one `state.json` and one set of verbs. Each gets its own page.

## Refreshing the assessment as you build
...
|---|---|---|
| A PR is up for review | `ciso:review` | Reads the diff against the controls you track and reports which ones it supports and which it might **regress**. Also flags open gaps in the same area. Read-only. It never records anything. |
...
| Before an audit, or periodically | `ciso:audit` | Flags what an auditor would reject. That covers controls marked met with **no evidence** or only thin justifications, and assessments over 12 months old. It also covers unjustified exclusions and controls never assessed. For ISO 27001 it also produces the draft Statement of Applicability. |

`ciso:evidence` deliberately **never changes a control's status.** Assessment (the claim) and
evidence (what backs it up) are independent axes. That's exactly what lets `ciso:audit` surface
the interesting case: a control claimed as met with nothing behind it. Changing a status is always
`ciso:interview`.

This tool never runs anything in CI. `state.json` is gitignored and stays that way; CI output is
something you feed in from your own machine.
...
- **License-restricted content never lives in this repo.** HITRUST's verbatim requirement-statement
  ...
  **CMMC is the deliberate exception, and it is not a loophole:** NIST states in each of its
  ...
  regulations are US Government works, so that module includes the real requirement text. Note the
