...
This is a solo/small-maintainer open-source project, so expect an acknowledgement
on a best-effort basis, without a committed SLA. <!-- CONSULT: owner to confirm or
...
This repository includes prompt/skill content and local scripts (Python, Node.js)
...
and has no credentials of its own. Never commit secrets. See `.gitignore`.
